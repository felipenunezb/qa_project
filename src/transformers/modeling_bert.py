...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None):
        ...
        print(f'cls {input_ids.index(101)}')
        print(f'sep {input_ids.index(102)}')
        print(f' all sep: {[i for i,x in enumerate(input_ids) if x==102]}')
...
class SGBertEmbeddings(nn.Module):
    """Construct the embeddings from word, position and token_type embeddings.
    """
...
class BertEncoderS(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, encoder_hidden_states=True):
        all_encoder_layers = []
        for layer_module in self.layer:
            hidden_states = layer_module(hidden_states, attention_mask)
            #print('size of hidden_states in BertEncoder is',hidden_states.shape)
...
class BertModelE(BertPreTrainedModel):
    """
...
class BertForQuestionAnsweringVQAPool_MultiVote(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
...
class BertForQuestionAnsweringVanillaSG(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.num_choices = 2913
...
class BertForQuestionAnsweringSteroids(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.num_choices = 2913
...
class BertForQuestionAnsweringEnriched(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.num_choices = 2913
