...
class ElectraModelS(ElectraPreTrainedModel):
...
class ElectraModelE(ElectraPreTrainedModel):
    ...
    def forward(
        ...
        #Concat regular embedding plus 128 objects scene graph embedding
        hidden_states = torch.cat((hidden_states, scenedata), dim=1)   #dim (batch, seq_len + 150, hidden)
...
class ElectraForQuestionAnsweringVQA(ElectraPreTrainedModel):
    config_class = ElectraConfig
    base_model_prefix = "electra"
...
class ElectraForQuestionAnsweringVQAPool(ElectraPreTrainedModel):
    config_class = ElectraConfig
    base_model_prefix = "electra"
    ...
    def forward(
        ...
        if start_positions is not None and end_positions is not None:
            ...
            #print(orig_answers.clamp_(0, ignored_index).size())
...
class ElectraForQuestionAnsweringVQAPool_MultiVote(ElectraPreTrainedModel):
    config_class = ElectraConfig
    base_model_prefix = "electra"
    ...
    def forward(
        ...
        voter_1 = torch.mean(discriminator_hidden_states[1][-2], dim=1) 
...
class ElectraForQuestionAnsweringSteroids(ElectraPreTrainedModel):
    config_class = ElectraConfig
    base_model_prefix = "electra"
    ...
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.electra = ElectraModelS(config)
        self.decoder = BertDirectedAttention(config)
        self.hidden_size = config.hidden_size
        self.qa_outputs_start = nn.Linear(config.hidden_size, 1)
        self.qa_outputs_end = nn.Linear(config.hidden_size,1)
...
class ElectraForQuestionAnsweringEnriched(BertPreTrainedModel):
    ...
    def forward(
        ...
        #mean of second to last layer of hidden states
        voter_1 = torch.mean(outputs[1][-2], dim=1)
        #mean of last layer of hidden states
        voter_2 = torch.mean(outputs[1][-1], dim=1)
        #mean of both voters
        #voter_3 = torch.mean(torch.stack([voter_1, voter_2]), dim=0)
        voter_3 = torch.mean(outputs[1][-3], dim=1)
        #max of both voters
        #voter_4 = torch.max(voter_1, voter_2)
        voter_4 = torch.mean(outputs[1][-4], dim=1)
